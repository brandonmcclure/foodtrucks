...
echo "SQL UP!"
/opt/mssql-tools/bin/sqlcmd -S localhost -V16 -U sa -P $SA_PASSWORD -l 300 -d master -i /mnt/init.sql
rc=$?; if [[ $rc != 0 ]]; then echo "ini step failed with code $rc" && exit $rc; fi